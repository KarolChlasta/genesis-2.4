...
## RESOLVED (2026-07-25): multi-compartment GPU tridiagonal (Hines) solve implemented and validated

The "Limitation: single-compartment networks only" section below (and the
`~96x` multicompartment figure that appeared in early highlights/notes
before this fix) described `chip_channel_multiloop`'s known defect: it
updates each compartment independently, so it silently drops axial
(Hines) coupling for any real dendritic tree. **That gap is now closed.**

A new kernel, `hines_tree_eliminate` (OpenCL: `opencl/ocl_channel.cl`;
CUDA: `cuda/cuda_channel.cuh`), performs the actual tridiagonal Hines
elimination on the GPU -- one work-item/thread per disconnected tree
(neuron), reading/writing the same flat `funcs[]`/`ravals[]`/`results[]`
opcode-program buffers the CPU solver already builds, so each GPU thread
runs the identical bottom-up/top-down elimination algorithm the CPU
reference does, not an approximation of it. Host-side dispatch
(`ocl_hsolve.c`/`cuda_backend.cu`+`cuda_hsolve.c`) runs `nsteps` iterations
of (channel-kinetics kernel, then eliminate kernel) inside one multiloop
batch, so the "no per-step host round trip" property that made the
single-compartment kernel fast is preserved for real dendritic trees too.

**Correctness**: validated to ~1e-7 V agreement against the CPU (fp64)
reference across linear-chain topologies of arbitrary depth (NCOMP 1-32),
branching topologies (multiple dendrite branches per soma), and the
degenerate single-compartment case -- on CPU, on a local AMD iGPU
(OpenCL), and on UMCS cluster inf02 (A40, OpenCL+CUDA) and inf03 (A100,
CUDA). One narrow, deliberately-unfixed gap remains: a branch that is a
single compartment directly off the soma, combined specifically with the
hsolve's first-discovered tree, hits an explicit "unhandled" diagnostic
rather than computing something silently wrong (real dendrite models
essentially never use zero-length branch stubs, so this was judged low
value to chase further). Full derivation, the several genuinely subtle
bugs found and fixed while building this (each of which would have
produced a *silently* wrong GPU kernel if missed), and the validation
trail: `genesis/src/hines/GPU_HINES_SOLVE_DESIGN.md`.

**Speed** (real cluster hardware, not the integrated GPU used for the
single-compartment numbers elsewhere in this plan/the paper): a
systematic N x NCOMP sweep, `hh_multicompartment_benchmark.g`, CPU vs.\
GPU tree-elimination multiloop --

| N | NCOMP | total comps | A40 speedup | A100 speedup |
|---:|---:|---:|---:|---:|
| 100 | 16 | 1\,600 | 0.10x (slower) | 0.24x (slower) |
| 1\,000 | 16 | 16\,000 | 1.46x | 3.19x |
| 5\,000 | 16 | 80\,000 | 6.43x | 14.05x |
| 10\,000 | 16 | 160\,000 | **12.70x** | **29.08x** |

Same qualitative shape as the single-compartment kernel's own scaling
curve: fixed per-dispatch overhead makes the GPU *slower* than CPU at
small problem sizes, crossing over and growing steadily faster with size.
The A100 is ahead of the A40 at every matched point past the smallest
sizes, and pulls further ahead as the problem grows (more SMs/bandwidth
paying off only once there is enough parallel work). Full two-axis
sweep, raw CSVs, and reading notes: `cluster_bringup/logs/
MULTICOMPARTMENT_SWEEP_2026-07-25_ANALYSIS.md`.

**One engineering finding worth flagging**: the multiloop dispatch
pattern (many back-to-back kernel launches with no host sync within a
batch) triggered a reproducible driver-level hang on the local AMD
iGPU above a sharp compartment-count threshold (~22\,400 working,
~24\,000 hanging) -- confirmed, by testing the *exact* hanging
configuration on cluster hardware, to be specific to that one
integrated/display-sharing GPU and **not reproduced on either A40 or
A100** (tested up to 160\,000 compartments there without issue). A
conservative, overridable safety cap (`GENESIS_OCL_TREE_MAX_NCOMPTS`,
default 20\,000, disabled on cluster runs) guards the untested case
without blocking validated cluster use.

**Still open**: a finer-grained speed sweep (this one used a first,
reasonable N x NCOMP grid, not an exhaustive one); the same statistical
rigor (multi-replicate, confidence intervals) the single-compartment
benchmarks elsewhere in this plan already have; inf04 (4x A40)/inf05
(H200) not yet tried; branching-topology *speed* (only correctness
validated so far).
