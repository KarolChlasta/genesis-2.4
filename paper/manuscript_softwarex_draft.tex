...
\begin{enumerate}
\item An opt-in OpenCL backend and a CUDA backend for the \texttt{hsolve} channel-update path, behind one entry point, leaving every existing CPU, MPI, model and SLI-script workflow unchanged.
\item \hte{}, a GPU tridiagonal (Hines~\cite{hines1984efficient}) elimination kernel that extends acceleration from isopotential cells to axially-coupled multi-compartment dendritic trees, running the identical elimination the CPU solver does.
\item Four defects fixed in the accelerator that produced silently wrong results, and three in the Hines solver itself that suppressed \texttt{inject}-driven voltage transients in any multi-neuron \texttt{hsolve} --- the latter independent of GPU use and therefore affecting existing GENESIS users.
\item A latent $O(n^2)$ model-construction bottleneck, unrelated to accelerator support, reduced to linear; populations that previously did not finish building now do so in under a minute.
\item A head-to-head evaluation against NEURON and CoreNEURON on a published network model, with the equivalence of the two implementations established rather than assumed, and a benchmarking protocol in which both arms are timed identically throughout.
\end{enumerate}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item A GPU tridiagonal (Hines) elimination kernel, \hte{} (OpenCL and CUDA), extending acceleration to real, axially-coupled multi-compartment dendritic trees (Section~3).
...
This ordering is worth stating plainly, because it is not the one a reader would assume. CoreNEURON exists specifically to make NEURON models run faster: it discards the interpreter, rebuilds the data layout for cache efficiency and vectorisation, and is the engine used for NEURON's largest published simulations. GENESIS~2.5, by contrast, is a 2.4-era codebase with an accelerator added to one path, and the arm measured here does not use that accelerator at all --- it is the plain CPU solver. That it still finishes the same network in two thirds of CoreNEURON's time indicates that GENESIS's tabulated-channel CPU solver is competitive with a purpose-built optimised engine, which is not a claim the previous version of this manuscript was in a position to make.

CoreNEURON is in turn $1.25\times$ faster than NEURON~9.0.2, the expected ordering, which indicates the CoreNEURON arm is configured correctly rather than accidentally handicapped.
...
Packaging into a container-based on-premise/cloud deployment pipeline~\cite{chlasta2023pipeline} is in progress.
