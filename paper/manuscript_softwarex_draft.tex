...
\textbf{GPU against a GPU-native simulator.} The comparison above is CPU on both sides, because the Vogels--Abbott network is spiking and the accelerator declines it (Section~2.1). To test the accelerator across simulators we used a model it does support --- $N=10\,000$ neurons, each a chain of 16 compartments, Hodgkin--Huxley channels, current injection, no synapses --- reimplemented with matching geometry, conductances, injection and timestep in NEURON and in Arbor~\cite{arbor2019}, which was written from scratch for many-core and GPU hardware.

Over $K=5000$ steps on one node (Table~\ref{tab:mccross}), GENESIS~2.5 on the A100 finishes in $1.75\pm0.08$\,s against Arbor's $1.56\pm0.02$\,s: Arbor is $1.12\times$ faster end-to-end and $1.26\times$ faster on the simulation phase alone. We do not claim to beat it. The claim is that a simulator whose solver predates GPUs, carrying every existing model and SLI script unchanged, lands within 12\% of a GPU-native design. The device profile locates the gap: the kernels take $82.3\,\mu$s per step against a step phase of $123.1\,\mu$s, so a third of our simulation time is host-side. The kernel alone ($0.41$\,s) is faster than Arbor's whole simulation phase; what costs us the comparison is per-step dispatch, not arithmetic. This is the cost of the design that keeps existing models working: Arbor generates specialised code per mechanism, while our kernels interpret the \texttt{hsolve} opcode program at run time. On single-threaded CPU the GENESIS solver is $2.02\times$ slower than CoreNEURON, so it is the accelerator, not the solver, that this release contributes. Arbor is also faster on 128 CPU threads than on the GPU at this size, its construction cost dominating: an accelerator's advantage is regime-dependent.
...
\emph{Comparison.} CoreNEURON is designed for GPU and multi-rank MPI execution; measuring those configurations, and Arbor on a spiking network, would place GENESIS~2.5 more completely.
