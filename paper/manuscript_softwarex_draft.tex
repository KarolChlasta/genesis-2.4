...
\begin{abstract}
GENESIS is a long-established compartmental neural simulator. We release GENESIS~2.5, adding opt-in OpenCL and CUDA backends for the Hodgkin--Huxley channel-update path and \hte{}, a GPU Hines tree-elimination kernel for dendritic trees, leaving every existing model, script and MPI workflow unchanged. On NVIDIA A100 hardware it reaches $57\times$ end-to-end speedup on a 160\,000-compartment population, matching the fp64 CPU to $\sim$$10^{-7}$\,V. It runs the Vogels--Abbott COBAHH benchmark $1.63\pm0.07\times$ faster than CoreNEURON, and overtakes the GPU-native Arbor beyond 66\,ms of simulated time, its cost per step being 34\% lower. The release also fixes seven latent defects and makes $O(n^2)$ model construction linear.
\end{abstract}
...
\caption{GPU wall clock against run length for the multi-compartment benchmark, $N=10\,000$ neurons $\times$ 16 compartments on the UMCS A100, mean $\pm$ std over 3 replicates. Both simulators are linear in $K$ with different intercepts and slopes: Arbor starts $0.36$\,s ahead on setup, GENESIS~2.5 advances each step $33\%$ more cheaply. Neither is faster in general --- the lines cross at $K\approx11\,100$, and a comparison at a single run length reports whichever side of that the author happened to choose.}
\label{fig:crossover}
\end{figure}
