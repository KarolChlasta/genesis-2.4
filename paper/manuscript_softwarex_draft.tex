...
\begin{abstract}
GENESIS is a long-established compartmental neural simulator; PGENESIS extends it to MPI. We release GENESIS~2.5, adding opt-in OpenCL and CUDA backends for the Hodgkin--Huxley channel-update path and \hte{}, a GPU Hines tree-elimination kernel for axially-coupled dendritic trees, leaving every existing CPU, MPI, model and SLI-script workflow unchanged. On NVIDIA A40/A100 hardware it reaches $21\times$ end-to-end speedup for single-compartment networks and $57\times$ for 160\,000-compartment populations at realistic run lengths, matching the fp64 CPU to $\sim$$10^{-7}$\,V. On the Vogels--Abbott COBAHH benchmark GENESIS~2.5 runs $1.63\pm0.07\times$ faster than CoreNEURON. The release also fixes seven latent defects, including silently wrong accelerator results, and reduces $O(n^2)$ model construction to linear.
\end{abstract}
...
Over $K=5000$ steps on one node (Table~\ref{tab:mccross}), GENESIS~2.5 on the A100 finishes in $1.75\pm0.08$\,s against Arbor's $1.56\pm0.02$\,s, so Arbor is $1.12\times$ faster end-to-end. We do not claim to beat it. The claim is that a simulator whose solver predates GPUs, running every existing model and script unchanged, lands within 12\% of a GPU-native design. The device profile locates the gap: warm, the kernels take $64.3\,\mu$s per step against a step phase of $123.1\,\mu$s, so about half of our simulation time is spent outside them. The kernels alone ($0.32$\,s) are faster than Arbor's whole simulation phase ($0.49$\,s); what costs us the comparison is per-step host work, not arithmetic. This is the cost of the design that keeps existing models working: Arbor generates specialised code per mechanism, while our kernels interpret the \texttt{hsolve} opcode program at run time, and GENESIS still walks its scheduler once per step even when one dispatch has advanced the whole batch. On single-threaded CPU the GENESIS solver is $2.02\times$ slower than CoreNEURON: it is the accelerator, not the solver, that this release contributes. Arbor is also faster on 128 CPU threads than on the GPU at this size, its construction cost dominating.
...
\hline
Arbor 0.10.0 & \textbf{GPU (A100)} & \textbf{1.56 $\pm$ 0.02} \\
\hline
GENESIS 2.5 & \textbf{GPU (A100)} & \textbf{1.75 $\pm$ 0.08} \\
\hline
...
\hline
\end{tabular}
\caption{Multi-compartment Hodgkin--Huxley benchmark across simulators: $N=10\,000$ neurons $\times$ 16 compartments (160\,000 compartments), $K=5000$ steps, $dt=0.01$\,ms, all on UMCS node inf03 (Xeon Platinum 8358, NVIDIA A100), mean $\pm$ std over 3 replicates, both arms of every pair timed identically. Thread counts differ by simulator and are stated: Arbor uses all cores by default, the others are single-threaded. The models match in geometry, passive properties, conductances, injection and timestep; this is a throughput comparison and not a claim that the three produce identical voltages. Harness and raw data: \protect\path{cluster_bringup/coreneuron/}.}
\label{tab:mccross}
\end{table}
