...
\item Seven defect fixes: four in the accelerator, which produced silently wrong results, and three in the Hines solver, which suppressed \texttt{inject}-driven transients in any multi-neuron \texttt{hsolve} --- the latter affecting existing users regardless of GPU use.
\item A latent $O(n^2)$ model-construction bottleneck, unrelated to accelerator support, reduced to linear.
\item A head-to-head evaluation against NEURON and CoreNEURON on a published network model, with equivalence established rather than assumed, and both arms timed identically throughout.
\end{enumerate}
...
These are \emph{end-to-end} speedups: both arms are wall-clocked around the whole process, so construction, transfers and per-step host work are inside the measurement. This matters more than it may appear. An earlier campaign took the CPU number from GENESIS's step-loop timer and the GPU number from the device-side kernel report --- two different quantities --- giving $212.7\times$ on the A40 at $N=50\,000$ where the end-to-end figure is $21.0\times$. It also manufactured an ordering between the cards, the A40 appearing to beat the A100 at every $N$, which invited a hardware explanation; measured consistently the two are indistinguishable ($21.0\pm0.3\times$ vs.\ $21.9\pm0.3\times$).
...
\emph{Correctness.} Linear-chain and branching benchmarks (the latter exercising sibling-array opcodes) compare GPU against fp64 CPU across \texttt{NCOMP}\,$=1$--$32$, up to 4 branches and $N=10\,000$ neurons: agreement is $\sim$$10^{-7}$\,V, matching single-compartment fidelity. One topology is not handled --- a single-compartment branch off a branch point, violating the kernel bootstrap's seed-row assumption. It had been documented but not enforced, so such a model was accelerated and returned wrong voltages silently; both backends now detect it at initialisation and compute that \texttt{hsolve} on the CPU, on the same pattern used for unsupported opcodes.
...
\textbf{Head-to-head against NEURON and CoreNEURON.} The speedups above are internal (GPU vs.\ CPU within GENESIS) and say nothing about how GENESIS compares with other simulators. We therefore ran the Vogels--Abbott COBAHH network~\cite{vogels2005signal,brette2007simulation} in GENESIS (\path{Scripts/VAnet2}) and in the reference NEURON implementation (ModelDB~83319, \path{NEURON/cobahh}) on the same cluster node, all arms single-threaded on CPU (Table~\ref{tab:coreneuron}). GENESIS~2.5 completes the 5.0\,s simulation in $46.9\pm2.1$\,s, $1.63\pm0.07\times$ faster than CoreNEURON and $2.04\pm0.09\times$ faster than NEURON~9.0.2. In throughput terms the run advances $4\times10^{8}$ neuron-steps, so GENESIS sustains $8.5\times10^{6}$ neuron-steps/s against $5.2\times10^{6}$ for CoreNEURON and $4.2\times10^{6}$ for NEURON.

The ordering is not the one a reader would assume. CoreNEURON exists to make NEURON models faster --- it discards the interpreter and rebuilds the data layout for vectorisation --- while the GENESIS arm measured here uses no accelerator at all, only the plain CPU solver. CoreNEURON is in turn $1.25\times$ faster than NEURON~9.0.2, the expected ordering, indicating that arm is configured correctly rather than handicapped.

Because these are independent implementations, equivalence was established rather than assumed: both networks have 4000 single-compartment cells (3200 excitatory, 800 inhibitory) with 80 synapses each, both are driven externally for only the first 50\,ms and then run on recurrent activity, and both use $dt=0.05$\,ms for 5.0\,s. Critically, both settle into the same regime --- $26.8$\,Hz mean rate in GENESIS against $27.9$\,Hz in NEURON --- so neither does materially less synaptic work. Obtaining the GENESIS rate required instrumenting every spikegen, since the benchmark records only three cells, two of them underconnected grid-edge cells firing at $7.6$ and $0.6$\,Hz against $15.8$\,Hz centrally.

The stock benchmark cannot run under CoreNEURON at all: its channels are ChannelBuilder objects rebuilt inside the interpreter, so CoreNEURON aborts in \texttt{nrn\_setup}. We transcribed them into NMODL, exactly reproducing the published COBAHH rates at $V_T=-63$\,mV, after which CoreNEURON yields a spike train byte-identical to NEURON's. Harness, mechanisms and full method are in \path{cluster_bringup/coreneuron/}.
...
Bringing up the backends surfaced four confounds --- a GUI-linked binary, a benchmark that never dispatches to the kernel, CPU-timer semantics that count GPU-blocked wait as compute, and an off-by-one \texttt{argc} guard --- each of which produced a false reading at some point. The wall-clock protocol built to catch them should generalize to any future GENESIS accelerator backend. The dominant early cost was a broken $O(N)$ scheduler loop re-scheduling neurons $2\ldots N$ every step; fixing it is what makes Section~3's numbers possible.

The accelerator's scope is worth stating precisely. Both kernels interpret the \texttt{hsolve} opcode program directly and implement the five opcodes a \texttt{tabchannel} model driven by current injection produces. Compartments needing anything else --- synaptic channels, a \texttt{spike} element, GHK, concentration pools --- are detected at \texttt{SETUP} and computed on the CPU. Validating this on a published network model exposed a fourth latent defect: the detection existed but its result was never consulted, so such compartments reached a kernel that mis-parses their opcode stream, giving wrong voltages from the first step with no error raised. It affected none of the benchmarks here, since none uses those elements, but it affects any real network model that does. The check is now enforced, and the boundary it draws is the honest statement of what this release accelerates.
...
Measured against other simulators on a published network model, GENESIS~2.5 is $1.63\pm0.07\times$ faster than CoreNEURON and $2.04\pm0.09\times$ faster than NEURON~9.0.2 for single-threaded CPU execution of the Vogels--Abbott COBAHH benchmark, with the equivalence of the two implementations established rather than assumed (Section~3).
