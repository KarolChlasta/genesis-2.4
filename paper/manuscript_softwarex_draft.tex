...
\begin{abstract}
GENESIS is a long-established compartmental neural simulator. We release GENESIS~2.5, adding opt-in OpenCL and CUDA backends for the Hodgkin--Huxley channel-update path and \hte{}, a GPU Hines tree-elimination kernel for dendritic trees, leaving every existing CPU, MPI, model and SLI-script workflow unchanged. On NVIDIA A40/A100 hardware it reaches $21\times$ end-to-end speedup for single-compartment networks and $57\times$ for 160\,000-compartment populations, matching the fp64 CPU to $\sim$$10^{-7}$\,V. On the Vogels--Abbott COBAHH benchmark it runs $1.63\pm0.07\times$ faster than CoreNEURON, and on a multi-compartment population it overtakes the GPU-native Arbor for runs beyond 66\,ms of simulated time, its marginal cost per step being 34\% lower. The release also fixes seven latent defects, including silently wrong accelerator results, and makes $O(n^2)$ model construction linear.
\end{abstract}
...
Which simulator is faster depends on how long the run is, and the crossover is measurable. Over $K=5000$ steps on one node (Table~\ref{tab:mccross}) Arbor finishes in $1.56\pm0.02$\,s against our $1.75\pm0.08$\,s; over $K=50\,000$ the order reverses, $4.66\pm0.07$\,s against Arbor's $5.92\pm0.06$\,s. Both are linear in $K$: GENESIS costs $1.29$\,s fixed plus $64.5\,\mu$s per step, Arbor $1.07$\,s fixed plus $97.8\,\mu$s. Our marginal cost is $34\%$ lower and theirs starts $0.22$\,s ahead, so they cross at $K\approx6600$ --- 66\,ms of simulated time at $dt=0.01$\,ms. Below that Arbor wins on setup; above it, which covers most modelling studies, GENESIS~2.5 is faster. The device profile explains the split: our step phase exceeds kernel time by $0.21$\,s at $K=5000$ and by the same $0.21$\,s at $K=50\,000$, so that excess is one-time dispatch setup, not per-step work, and it is what costs us the short-run comparison.  This is the cost of the design that keeps existing models working: Arbor generates specialised code per mechanism, while our kernels interpret the \texttt{hsolve} opcode program at run time. On single-threaded CPU the GENESIS solver is $2.02\times$ slower than CoreNEURON: it is the accelerator, not the solver, that this release contributes. Arbor is also faster on 128 CPU threads than on the GPU at this size, its construction cost dominating.
