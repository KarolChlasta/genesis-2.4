...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{fig4_gpu_large_runtime_ci.png}
\caption{GPU per-step channel-update wall time with 95\% CI across $N = 100$--$2000$ neurons (ROCm 6.3.1, Radeon 890M).}
\label{fig:gpu_large_ci}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{fig5_gpu_large_scaling_trend.png}
\caption{GPU kernel and host-path scaling trend with neuron count ($N$). Kernel time (blue) grows sub-linearly; host-side dispatch overhead (orange) dominates at all tested sizes.}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{fig3_headless_error_lines.png}
\caption{Headless binary comparison: error lines between \texttt{genesis} (X11-linked) and \texttt{nxgenesis} (headless) runs, illustrating the construction-phase overhead confound identified in the first round of GPU benchmarking.}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{fig7_cpu_gpu_speedup.png}
\caption{CPU vs.\ GPU end-to-end speedup ratio (per-step dispatch mode) across $N$. Values below 1$\times$ indicate that per-step PCIe transfer overhead makes the GPU path slower than CPU for this $N$ range.}
