...
\author{Karol Chlasta\textsuperscript{1,2}, Grzegorz Marcin W\'ojcik\textsuperscript{3}\\[6pt]
\small\textsuperscript{1}Kozminski University, Warsaw, Poland\\[2pt]
\small\textsuperscript{2}WarsawIQ, Warsaw, Poland\\[2pt]
\small\textsuperscript{3}Department of Neuroinformatics and Biomedical Engineering, Maria Curie-Sk{\l}odowska University in Lublin, ul. Akademicka 9, 20-033 Lublin, Poland\\[4pt]
\small Corresponding author: Karol Chlasta, karol@chlasta.pl}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{fig1_mean_runtime_ci.png}
\caption{CPU baseline mean wall-clock runtime with 95\% confidence intervals (\texttt{nxgenesis}, 30 replicates per workload).}
\label{fig:cpu_mean_ci}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{fig2_runtime_boxplot.png}
\caption{CPU baseline runtime distributions (boxplot). Whiskers extend to 1.5\,IQR; low spread (CV $\approx$1\%) confirms stable CPU timing.}
\label{fig:cpu_boxplot}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{fig6_cpu_dense_runtime_ci.png}
\caption{CPU dense-network benchmark: mean wall-clock runtime with 95\% CI across $N$ neurons (\texttt{nxgenesis}).}
\label{fig:cpu_dense_ci}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.80\textwidth]{fig4_gpu_large_runtime_ci.png}
\caption{GPU multiloop step-loop runtime with 95\% CI (fp32, Mesa rusticl, Radeon 890M, $n=30$ replicates, $K=5000$ steps). Individual replicate times are shown as translucent scatter; the line is the per-$N$ mean; the shaded band is the 95\% CI.}
\label{fig:gpu_large_ci}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{fig5_gpu_large_scaling_trend.png}
\caption{Normalized cost (runtime per neuron, s/neuron) for the GPU multiloop (fp32) and CPU (fp64) arms across $N$. GPU cost per neuron peaks at $N=500$ and decreases to a minimum near $N=5000$ before rising again at $N=20000$, reflecting kernel launch fixed cost at small $N$ and memory bandwidth saturation at large $N$.}
\label{fig:gpu_scaling_trend}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.60\textwidth]{fig3_headless_error_lines.png}
\caption{Mean stderr diagnostic lines per run for \texttt{nxgenesis} (headless mode). Cal8.g emits substantially more diagnostic output ($\approx$110 lines/run) than Cal7difshell.g ($\approx$17 lines/run); neither run produced errors or non-zero exit codes.}
\label{fig:headless_error}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.80\textwidth]{fig7_cpu_gpu_speedup.png}
\caption{GPU multiloop end-to-end speedup (CPU fp64 / GPU fp32, $n=30$ replicates). Speedup peaks at $14.8\times$ for $N=5000$ neurons and drops at $N=20000$ where GPU memory bandwidth saturates. Dashed line marks the 1$\times$ breakeven.}
\label{fig:cpu_gpu_speedup}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{fig8_ocl_multiloop_runtime_ci.png}
\caption{OCL multiloop benchmark: GPU dispatch (fp32, Mesa rusticl) and CPU baseline wall-clock time with 95\% CI across $N$, post-scheduler-fix.}
\label{fig:multiloop_ci}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{fig9_ocl_multiloop_speedup.png}
\caption{OCL multiloop end-to-end speedup (GPU fp32 / CPU fp64) after scheduler fix. Speedup rises from 3.2$\times$ at $N=500$ to 16.2$\times$ at $N=5000$.}
