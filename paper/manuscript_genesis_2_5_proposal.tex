...
path. The multiloop kernel's correctness under spiking conditions remains open work.
...
One new finding arose during verification: the \texttt{chip\_channel\_multiloop} kernel
gives a different, subthreshold result at all $N > 1$. This is a separate correctness
bug, isolated to that kernel's inlined Hines solve, which is independent of the three
bugs above. It does not affect the timing results in Table~\ref{tab:multiloop} or
Table~\ref{tab:multiloop_fp32} (kernel dispatch time is independent of whether the
simulated voltage happens to cross threshold), but it means the multiloop kernel's
spiking correctness is not yet established.
