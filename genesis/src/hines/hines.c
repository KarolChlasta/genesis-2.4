	    ...
	    case PROCESS:
			...
			case 5:
				...
				if (BaseObject(hsolve)->method == CRANK_INT)
				    ...
				    do_euler_hsolve(hsolve);
				/* only meaningful when do_euler_hsolve/do_crank_hsolve
	...
	Hsolve	*hsolve;
{
	...
	if (hsolve->ncompts > 0) {
	    ...
	    if (hsolve->fwd_seg_start) { free(hsolve->fwd_seg_start); hsolve->fwd_seg_start=NULL; }
	    if (hsolve->bwd_seg_start) { free(hsolve->bwd_seg_start); hsolve->bwd_seg_start=NULL; }
	    if (hsolve->fwd_root_row) { free(hsolve->fwd_root_row); hsolve->fwd_root_row=NULL; }
	    if (hsolve->fwd_raval_start) { free(hsolve->fwd_raval_start); hsolve->fwd_raval_start=NULL; }
	    if (hsolve->bwd_raval_start) { free(hsolve->bwd_raval_start); hsolve->bwd_raval_start=NULL; }
	    hsolve->n_trees=0;
            if (hsolve->ravptr) {
                for (i=0;i<hsolve->ncompts;i++) {
