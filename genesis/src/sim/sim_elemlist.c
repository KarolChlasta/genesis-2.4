...
int	new_size;
    ...
    if(list != NULL){
	/*
	** Grow geometrically (double) once the list is bigger than the
	** fixed increment, instead of always adding a fixed increment.
	** A fixed increment (LIST_RESIZE=100) makes building an n-element
	** list via repeated AddElementToList() calls cost O(n^2) total
	** copying once n is much bigger than the increment -- exactly
	** what happens for a big wildcard match, e.g.
	** `useclock /net/##[] 0` over a large population.
	*/
	new_size = list->size + expansion_size;
	if(list->size * 2 > new_size)
	...
	*/
	if(list->element){
	    ...
	    realloc(list->element, sizeof(Element *)*new_size);
	} else {
	    list->element  = (Element **)
	    calloc(1, sizeof(Element *)*new_size);
	}
	list->size = new_size;
    }
}
