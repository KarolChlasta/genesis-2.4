...
// Solver channel mode: GENESIS_BENCH_CHANMODE overrides (default 4 = accelerator
// path used by the GPU arm; set to 1 for a real CPU-Hines reference). This makes
// a fair CPU-vs-GPU compute comparison possible.
// Integer-typed getenv check (not str != ""): an unset var's string form casts
// to a NULL/"" that trips "CastToStr: NULL string" / "CastToInt" errors and can
// leave CHANMODE at the wrong value silently (found 2026-07-25 -- this exact
// bug previously fixed the same way in hh_multicompartment_benchmark.g).
int CHANMODE = 4
...
echo "Configuring single hsolve (chanmode=" {CHANMODE} ", calcmode=1, N=" {N_NEURONS} " neurons)..."
...
// walltimemark/{walltime}: single-process, microsecond-resolution wall-clock
// step-phase timer (GENESIS 2.5, added 2026-07-25; see sim/sim_ops.c). Replaces
// the old {cpu} pair, which is a void function coerced to 0 (always read
// 0 s -- see cluster_bringup/BENCHMARK_NOTES.md) and, being getrusage/CPU-time
// based, would in any case be blind to GPU-side compute in the CUDA/OpenCL
// arms. This also avoids the external K=0-vs-K=large two-process subtraction
// method's cross-run noise (see cluster_bringup/logs/CAMPAIGN_2026-07-24_run2_status.md).
//
// IMPORTANT (found 2026-07-25): in OCL/CUDA multiloop mode, the FIRST `step`
// call after multiloop is configured dispatches the ENTIRE configured
// GENESIS_{OCL,CUDA}_MULTILOOP step count in one GPU batch, regardless of how
// many steps THAT call nominally requested -- every subsequent `step` call is
// then a no-op ("CPU Hines identity pass-through"). A separate warm-up `step
// 10` before the timed region (the old structure here) therefore silently ate
// the entire GPU dispatch, leaving the "timed" call measuring nothing. Fix:
// one single `step` call covers both the settle-in steps and the measured
// steps, timed as one block -- consistent for the CPU (chanmode=1, where step
// calls execute literally/sequentially, so combining them changes nothing) and
// GPU (chanmode=4, where this is the only structure that measures real work)
// arms alike. Set GENESIS_{OCL,CUDA}_MULTILOOP = N_STEPS+10 to match.
walltimemark
step {N_STEPS + 10}
